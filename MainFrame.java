...
public class MainFrame extends JFrame {
    ...
    JPanel buildLoginScreen() {
        ...
        JButton diagBtn   = btn("🔧 Test DB Connection", new Color(30, 58, 80));

        loginBtn.setAlignmentX(CENTER_ALIGNMENT);
        regBtn.setAlignmentX(CENTER_ALIGNMENT);
        diagBtn.setAlignmentX(CENTER_ALIGNMENT);
        ...
        diagBtn.setMaximumSize(new Dimension(Integer.MAX_VALUE, 36));
        diagBtn.setFont(FONT_SMALL);
    ...
    void runDiagnostics() {
        ...
        sb.append("=== DB Diagnostics ===\n\n");


        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            sb.append("✅ MySQL JDBC Driver found\n");
        } catch (ClassNotFoundException ex) {
            sb.append("❌ MySQL JDBC Driver NOT found\n");
            sb.append("   → Add mysql-connector-j.jar to your classpath\n\n");
            showDiagResult(sb.toString());
            return;
        }
        ...
        try {
            ...
            sb.append("✅ Connected to MySQL server\n");
        } catch (java.sql.SQLException ex) {
            sb.append("❌ Cannot connect to MySQL\n");
            sb.append("   Error: ").append(ex.getMessage()).append("\n\n");
            sb.append("Common causes:\n");
            sb.append("  • MySQL server is not running\n");
            sb.append("  • Wrong password in DBConnection.java\n");
            sb.append("  • Wrong port (default 3306)\n");
            showDiagResult(sb.toString());
            return;
        }
        ...
        try {
            ...
            boolean found = false;
            while (rs.next()) {
                if (rs.getString(1).equalsIgnoreCase("findmyroomie")) { found = true; break; }
            }
            if (found) sb.append("✅ Database 'findmyroomie' exists\n");
            else {
                sb.append("❌ Database 'findmyroomie' does NOT exist\n");
                sb.append("   → Run: CREATE DATABASE findmyroomie;\n");
            }
        } catch (java.sql.SQLException ex) {
            sb.append("⚠️  Could not check databases: ").append(ex.getMessage()).append("\n");
        }


        String[] tables = {"users", "rooms", "applications"};
        for (String table : tables) {
            try {
                java.sql.Statement st = conn.createStatement();
                st.executeQuery("SELECT 1 FROM " + table + " LIMIT 1");
                sb.append("✅ Table '").append(table).append("' exists\n");
            } catch (java.sql.SQLException ex) {
                sb.append("❌ Table '").append(table).append("' missing or error: ")
                  .append(ex.getMessage()).append("\n");
                sb.append("   → Run schema.sql to create tables\n");
            }
        }


        try {
            java.sql.ResultSet rs = conn.getMetaData().getColumns(null, null, "users", null);
            java.util.List<String> cols = new java.util.ArrayList<>();
            while (rs.next()) cols.add(rs.getString("COLUMN_NAME").toLowerCase());
            String[] needed = {"aadhar","smoking_habit","drinking_habit","occupation",
                               "gender","preferred_city","age","bio"};
            for (String col : needed) {
                if (cols.contains(col)) sb.append("✅ users.").append(col).append(" exists\n");
                else sb.append("❌ users.").append(col).append(" MISSING → re-run schema.sql\n");
            }
        } catch (java.sql.SQLException ex) {
            sb.append("⚠️  Could not inspect columns: ").append(ex.getMessage()).append("\n");
        }
    ...
    void showDiagResult(String text) {
        ...
        JOptionPane.showMessageDialog(this, sp, "DB Diagnostics", JOptionPane.PLAIN_MESSAGE);
    }
